...
export default class Generator {
  ...
  input(q) {
    ...
    else res = inquirer.input(Object.assign(getQOptions(this.params[q.name]), q));
  ...
  password(q) {
    ...
    //(2) prompt
    if (this.responses.hasOwnProperty(q.name)) res = this.responses[q.name];
    else res = inquirer.password(Object.assign(getQOptions(this.params[q.name]), q));
  ...
  exists(...entry) {
    ...
    else this.simple(params => this[exists](...params))(`Check whether '${new fs.entry(entry).replacePath(this.dst + "/")}' exists`, entry);
  }
